from gromacs_energy_plot import get_dowser_energies, get_gmx_energies
...
def get_removal_energy_and_std(cluster_energy_xvg, minus_one_energy_files):
    ...
    cluster_energy = np.mean(np.sum(get_energy_from_xvg(cluster_energy_xvg),
                                    axis=1))
    # cluster_energy = np.sum(get_energy_from_xvg(cluster_energy_xvg), axis=1)
    var_cluster_energy = np.var(np.sum(get_energy_from_xvg(cluster_energy_xvg),
                                       axis=1))
    std_cluster_energy = np.std(np.sum(get_energy_from_xvg(cluster_energy_xvg),
                                       axis=1))
    print(f"variance and std of initial cluster energy: {var_cluster_energy},\
            {std_cluster_energy}")
...
def plot_removal_energy_vs_site(removal_energies, gmx_energies, sites,
                                output_filename, std_gmx_energies,
                                dowser_energies, scaled_removal,
                                scaled_dowser, scaled_gmx):
    ...
    plt.plot(sites, gmx_energies, 'b^', label='gromacs potential',
             markersize=10)
    plt.plot(sites, scaled_removal, '^', color='limegreen',
             label='scaled removal', markersize=10)
    plt.plot(sites, scaled_gmx, '^', color='lightblue',
             label='scaled gromacs potential', markersize=10)
    ...
    higher_energies = np.array([x for x in sites_and_removal_energy
                                if x[1] > energy_threshold])
    print(f"{len(higher_energies)} water with energies higher than \
    ...
    if dowser_energies.any():
        ...
        plt.plot(sites, scaled_dowser, 'v', color='lightcoral',
                 label='scaled dowser', markersize=10)
